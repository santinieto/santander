...
class UnitTest(unittest.TestCase):
    ...
    def test_create_teachers(self, nteachers=1):
        ...
        for k in range(nteachers):
            ...
            self.assertEqual(str(response_data["username"]), teacher_data["username"])
            self.assertEqual(str(response_data["password"]), teacher_data["password"])
            self.assertEqual(str(response_data["first_name"]), teacher_data["first_name"])
            self.assertEqual(str(response_data["last_name"]), teacher_data["last_name"])
            self.assertEqual(str(response_data["nationality"]), teacher_data["nationality"])
            self.assertEqual(str(response_data["email"]), teacher_data["email"])
    ...
    def test_create_teacher_error(self):
        ...
        response = requests.post("http://127.0.0.1:8000/add_teacher/", json=invalid_teacher_data)
